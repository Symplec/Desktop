...
/* macros */
#define IS_WIN_SET(flag)		((win.mode & (flag)) != 0)
...
mousereport(XEvent *e)
{
	...
	if (e->xbutton.type == MotionNotify) {
		if (x == ox && y == oy)
			return;
		if (!IS_WIN_SET(MODE_MOUSEMOTION) && !IS_WIN_SET(MODE_MOUSEMANY))
			return;
		/* MOUSE_MOTION: no reporting if no button is pressed */
		if (IS_WIN_SET(MODE_MOUSEMOTION) && oldbutton == 3)
			return;
		...
		if (!IS_WIN_SET(MODE_MOUSESGR) && e->xbutton.type == ButtonRelease) {
			button = 3;
		} else {
			button -= Button1;
			if (button >= 3)
...
bpress(XEvent *e)
{
	...
	if (IS_WIN_SET(MODE_MOUSE) && !(e->xbutton.state & forcemousemod)) {
		mousereport(e);
		return;
	}
...
selnotify(XEvent *e)
{
	...
	do {
		...
		if (IS_WIN_SET(MODE_BRCKTPASTE) && ofs == 0)
			ttywrite("\033[200~", 6, 0);
		ttywrite((char *)data, nitems * format / 8, 1);
		if (IS_WIN_SET(MODE_BRCKTPASTE) && rem == 0)
